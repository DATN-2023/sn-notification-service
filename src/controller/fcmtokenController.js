...
module.exports = (container) => {
  ...
  const getFcmtokenById = async (req, res) => {
    try {
      const { id } = req.params
      if (id) {
        const fcmtoken = await fcmtokenRepo.getFcmtokenById(id)
        res.status(httpCode.SUCCESS).send(fcmtoken)
      } else {
        res.status(httpCode.BAD_REQUEST).end()
      }
    } catch (e) {
  ...
  const getFcmtoken = async (req, res) => {
    try {
      ...
      Object.keys(search).forEach(i => {
        const vl = search[i]
        const pathType = (Fcmtoken.schema.path(i) || {}).instance || ''
      ...
      const data = await fcmtokenRepo.getFcmtoken(pipe, perPage, skip, sort)
      const total = await fcmtokenRepo.getCount(pipe)
